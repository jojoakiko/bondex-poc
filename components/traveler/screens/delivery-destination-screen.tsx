...
interface DeliveryDestinationScreenProps {
  data: BookingData
...
export function DeliveryDestinationScreen({ data, onUpdate, onNext, onBack }: DeliveryDestinationScreenProps) {
  ...
  const canContinue = useMemo(() => {
    ...
    const basicInfo = !!(selectedFacility && arrivalDate && arrivalTime && bookingName && bookingDoc && hasRecipient)
    if (isAirport) return !!(basicInfo && flightNumber && logisticsStatus?.isPossible)
    return basicInfo
  }, [selectedFacility, arrivalDate, arrivalTime, bookingName, bookingDoc, sameAsBooking, recipientName, isAirport, flightNumber, logisticsStatus])
  ...
  return (
    <div className="flex-1 flex flex-col max-w-md mx-auto w-full pb-8 bg-background">
      <div className="p-4 flex items-center gap-4 border-b border-border sticky top-0 bg-white/80 backdrop-blur-md z-20">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight">Delivery Info</h1>
              ...
              <Popover open={datePickerOpen} onOpenChange={setDatePickerOpen}>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <DateCalendar
                    ...
                    onSelect={(d) => {
                      if (d) { setArrivalDate(formatYmdLocal(d)); setDatePickerOpen(false) }
                    }}
      ...
      <div className="p-4 bg-white border-t border-border mt-auto">
        ...
        >
          {logisticsStatus?.error ? "Schedule Error" : "Confirm & Continue"}
        </Button>
      </div>
    </div>
  )
